...
export async function sendAdminNotificationEmail(order: Order) {
  ...
  const mailOptions = {
    ...
    html: `
      ...
      <p><strong>Country:</strong> ${order.country}</p>
      <p><strong>Order Type:</strong> ${order.orderType}</p>
      <p><a href="${process.env.NEXT_PUBLIC_APP_URL}/admin/orders/${order.id}">View Order Details</a></p>
    `,
  };
...
export async function sendCatalogEmail(email: string, name: string, catalogType: string = 'catalog') {
